...
async function getMediumAvatarWithCache(username: string): Promise<string> {
  ...
  // Double-insurance static presets for absolute resilience and instant initial rendering
  const staticPreserves: { [key: string]: string } = {
  ...
  // Tier 1: Direct Fetch with high-fidelity agent headers (We proved this gets a 100% success response on Cloud Run)
  try {
